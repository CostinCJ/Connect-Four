from GameErrors import BoardError
...
class Board:
    ...
    def make_move(self, column: int, circle_color: str):
        ...
        :param column: column where the circle will be placed
    ...
    def final_state(self):
        ...
        :return: if the game is over, it returns the score of the winner, else it returns 0
        """
        ...
        if abs(self.__table[to_check] + self.__table[to_check + (self.__columns + 1)] + self.__table[
            to_check + (self.__columns + 1) * 2] + self.__table[
                   to_check + (self.__columns + 1) * 3]) == self.__win_score:
            return self.__table[to_check]
        start, finish = 3, 7
    ...
    def is_board_full(self):
        ...
        :return: returns True if the board is full, else it returns False
        """
        return self.__move_count == self.__size

    def possible_columns(self):
        """
        This function returns a list of columns that are not full.
        """
        columns_left = []
        for column in range(self.__columns):
            if self.__validator[column] != -1:
    ...
    def create_board(self):
        """
        This function creates a board for the user to see.
        """
        board = []
